...
def get_gl_entries(filters):
	...
	gl_entries = frappe.db.sql(
		...
		select
			...
			IFNULL(jv.primary_customer, IFNULL(si.primary_customer, IFNULL(pe.primary_customer, gl.party))) as primary_customer,
...
def get_conditions(filters):
	...
	if filters.get("finance_book"):
		if filters.get("include_default_book_entries"):
			conditions.append("(gl.finance_book in (%(finance_book)s, %(company_fb)s, '') OR gl.finance_book IS NULL)")
		else:
		...
		for dimension in accounting_dimensions:
			if filters.get(dimension.fieldname):
				if frappe.get_cached_value('DocType', dimension.document_type, 'is_tree'):
					...
					conditions.append("gl.{0} in %({0})s".format(dimension.fieldname))
				else:
					conditions.append("gl.{0} in (%({0})s)".format(dimension.fieldname))
