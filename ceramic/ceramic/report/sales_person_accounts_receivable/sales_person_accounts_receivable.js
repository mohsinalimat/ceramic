...
frappe.query_reports["Sales Person Accounts Receivable"] = {
	"filters": [
		{
			"fieldname":"company",
			"label": __("Company"),
	...
	onload: function(report) {
		...
		frappe.db.get_value('Sales Person', {'user':frappe.session.user}, "name", function(r) {
			if(r.name){
				frappe.query_report.set_filter_value('regional_sales_manager', r.name);
			}
		});
	}
}
...
function view_receivable_report(primary_customer, company) {
	...
	// });
	frappe.set_route("query-report", "Party Ledger Ceramic RSM",{
