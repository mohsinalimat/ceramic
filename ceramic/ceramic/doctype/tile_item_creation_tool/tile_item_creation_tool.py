...
class TileItemCreationTool(Document):
	...
	def create_items(self):
		...
		for category in categories:
			...
			# new_item_name = self.item_series.replace(self.item_series.split("-")[-1], category)
			...
			#item.item_series = self.item_series.replace(self.item_series.split("-")[-1], category)
			if self.item_series:
			...
			# 	# if self.item_series != item.item_code: 
			# 	# 	item.save(ignore_permissions=True)
			# 	# 	item.submit()
			...
			# with last word condition
			# if category != self.item_series.split("-")[-1]:
			# 	item = frappe.new_doc("Item")
			# 	new_item_name = self.item_series.replace(self.item_series.split("-")[-1], category)
			# 	item.item_group = self.item_group
			# 	item.tile_size = self.tile_size
			...
			# 	item.batch_number_series = "BTH-.posting_date.-.###"
			# 	if item.item_series:
			# 		item.item_series = item.item_series + "-" + category
			# 	item.item_code = self.item_series + "-" + self.tile_surface + "-" + self.tile_size + "-" + new_item_name
